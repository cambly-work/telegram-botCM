...
|-----------|------------------|---------------------|-----------------|---------------|
| `None` → `waiting_name` | Пользователь запускает `/start`, у пользователя отсутствуют имя/email/телефон | `RegistrationStates.waiting_name` | Запрос имени с кнопкой «Отмена» | Проверено unit-тестом `test_registration_smoke_flow` (переход и текст запроса email после имени). 【F:handlers.py†L6206-L6225】【F:tests/test_fsm_smoke.py†L119-L175】 |
| `waiting_name` | Ответ с именем | `RegistrationStates.waiting_email` | «Укажи email для связи» | Проверено в `test_registration_smoke_flow`: после имени состояние `waiting_email`, отправлен локализованный текст. 【F:handlers.py†L6253-L6265】【F:tests/test_fsm_smoke.py†L153-L164】 |
